...
import { fromHex, printHex, bytesEqual, decodeUTF8 } from "./util.js";
...
function poly1305_test() {
  ...
  for (var i = 0; i < testVectors.length; i++) {
    ...
    var out = Poly1305.auth(input, input.length, key);

    if (Poly1305.verify(expected, out) !== 1) {
      console.log('error: ', i);
      console.log('want:\n', expected.join(' '));
      console.log('got :\n', out.join(' '), '\n');  
    } else {
...
function aead_test1() {
  ...
  for (var i = 0; i < testVectors.length; i++) {
    ...
    var ret = AEAD.encrypt(key, nonce, plaintext, aad);
    ...
    ret = AEAD.decrypt(key, nonce, ret[0], aad, ret[1]);
